...
import { Scenario, ResourceCalculation as ResourceCalculatorType, Client } from '@/lib';
...
export default function ResourceCalculator({
  ...
  const [workOrders, setWorkOrders] = useState(initialData?.workOrders || 0);
  const [complexity, setComplexity] = useState(initialData?.complexity || 1);
  const [managerToClientRatio, setManagerToClientRatio] = useState(initialData?.managerToClientRatio || 3);
  const [specialistToWorkOrderRatio, setSpecialistToWorkOrderRatio] = useState(initialData?.specialistToWorkOrderRatio || 10);
  const [leadershipCount, setLeadershipCount] = useState(initialData?.calculatedHeadcount?.leadership || 0);
  const [specialistCount, setSpecialistCount] = useState(initialData?.calculatedHeadcount?.specialists || 0);
  const [associateCount, setAssociateCount] = useState(initialData?.calculatedHeadcount?.associates || 0);
  ...
  const calculateRecommendedStaffing = () => {
    ...
    const leadershipFromWorkOrders = Math.floor((workOrders / 1000) * complexityFactor);
    ...
    const specialistsFromWorkOrders = Math.floor((workOrders / 500) * complexityFactor);
    const specialistsFromComplaints = Math.floor((specialistToWorkOrderRatio / 100) * complexityFactor);
    const specialist = Math.max(2, specialistsFromWorkOrders + specialistsFromComplaints);
    
    // Calculate associates based on work orders
    const associate = Math.max(3, Math.floor((workOrders / 200) * complexityFactor));
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label htmlFor="workOrders" className="block text-sm font-medium text-gray-700 mb-1">
                  Work Order Volume
                </label>
                <input
                  type="number"
                  id="workOrders"
                  value={workOrders}
                  onChange={(e) => setWorkOrders(parseInt(e.target.value) || 0)}
                  min={0}
